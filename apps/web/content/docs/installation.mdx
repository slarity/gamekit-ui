...
GameKit UI is listed in the [official shadcn registry directory](https://ui.shadcn.com/docs/directory?q=gamekit), so install any game by short name — the CLI resolves the `@gamekitui` namespace automatically, no setup required:
